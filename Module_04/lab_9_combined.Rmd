...
```{r}
m2 <- lm(hygiene ~ cond2 + cond3, data = slp)
ols_regress(m2)
```
...
# Checking Math for intercept
#test <- aggregate(x=slp$hygiene, by = list(sex = slp$sex, cond = slp$cond), FUN = mean)
#test <- filter(test, sex == 0)
#mean(test$x)

#Checking math for female
#test2 <- aggregate(x=slp$hygiene, by = list(sex = slp$sex, cond = slp$cond), FUN = mean)
#test2
#
#test2 %>% 
#  group_by(sex) %>% 
...
Intercept:  The intercept is the grand mean of sleep hygiene among males (i.e., when x = 0) across all of the conditions.. 

sex.ffemale: This variable is involved in an interaction, so it's a simple slope. It is the predicted difference in average sleep hygiene between females and males across all of the conditions
