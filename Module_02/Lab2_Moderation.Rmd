...
plot_model(modINT, type = "pred", terms = c("att3", "att1"), show.intercept = TRUE)
...
```{r}
mem <- read_csv("memory.csv")
```



```{r}
ggplot(mem, aes(x = Dosage, y = Mem_Score)) +
  geom_jitter() + 
...
mem <- mutate(mem, 
                    ...
                    linear = ifelse(Dosage == 1,  -1, Dosage), 
                    linear = ifelse(Dosage == 2,   0, linear), 
                    linear = ifelse(Dosage == 3,   1, linear), 
                    
                    quadratic = ifelse(Dosage == 1,   1, Dosage),
                    quadratic = ifelse(Dosage == 2,  -2, quadratic),
                    quadratic = ifelse(Dosage == 3,   1, quadratic)
                    )
...
```{r}
m1 <- lm(Mem_Score ~ linear, data = mem)
ols_regress(m1)
```

```{r}
m2 <- lm(Mem_Score ~ linear + quadratic, data = mem)
ols_regress(m2)
```
