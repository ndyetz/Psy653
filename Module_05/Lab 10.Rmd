...
```{r}
log_mod1 <- glm(Y ~ X1, family = binomial, data = lr)
summary(log_mod1)
```
...
```{r}
log_mod2 <- glm(Y ~ X1 + X2, family = binomial, data = lr)
summary(log_mod2)
```
### Get OR and 95% confidence intervals
```{r}
...
# More complex model, 1 binary and 3 continuous variables
```{r}
...
```{r}
logreg_mod <- glm(bac_over ~ alcexp + pmood + weight_low + typ_drks, family = binomial, data = bac)
summary(logreg_mod)
```
...
Model 1 vs  Model 2: 
McFaddens R^2 increases from 0.209 to 0.227, indicating that adding pmood when controlling for alc_exp does not add much explained variance (~2%) to the model.

Model 2 vs. Model 3: 
R^2 increases from 0.227 to 0.244, indicating that adding weight_low when controlling for alcexp and pmood does not add much explained variance (~2%) to the model
